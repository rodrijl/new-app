...
const DriverList = ({ drivers }) => {
  ...
  const getDriverColor = (assignedPlaces) => {
    console.log("::asggg",assignedPlaces)
        ...
        const driverColor = getDriverColor(driver.assignedPlaces);
          return (
            <li key={driver.id} className={driverColor}>
              {driver.name}
              <p>Places assigned: {driver.assignedPlaces && driver.assignedPlaces.length > 0 ? driver.assignedPlaces.join(", ") : 'No places assigned'}</p>
            </li>
          );
        })}
      </ul>
